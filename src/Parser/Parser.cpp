...
int
Parser::get_binary_op_precedence(const Token& token)
{
...
Parser::parse_term()
{
	...
	case TokenType::T_IDENTIFIER:
		if (peek().has_value() && peek().value().Type == TokenType::T_IDENTIFIER) {
			...
			if (std::find_if(m_variables[m_current_scope_name].begin(), m_variables[m_current_scope_name].end(),
	...
	default:
		m_error->Error(token->LineNumber, token->ColumnNumber, token->PosNumber, "Unexpected token '{}'",
...
std::optional<Token>
Parser::peek(int ahead)
{
	if (m_index + ahead < m_tokens.size()) return m_tokens.at(m_index + ahead);
	return {};
}
...
std::optional<Token>
Parser::try_consume(TokenType type)
{
...
Token
Parser::must_consume(TokenType token)
{
	if (auto tok = try_consume(token)) {
		return tok.value();
	}
	m_error->Error(peek(-1).value().LineNumber, peek(-1).value().ColumnNumber, peek(-1).value().PosNumber,
				   "Expected token '{}' after '{}'", token_to_string(token), token_to_string(peek(-1).value().Type));
	exit(EXIT_FAILURE);
}
void
Parser::consume_semicolon(const std::unique_ptr<ASTNode>& statement)
{
...
void
Parser::add_variable(VariableDeclaration* var)
{
