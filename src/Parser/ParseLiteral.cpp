...
std::shared_ptr<NumberLiteral> Parser::parse_number_literal()
{
		...
		peek().value().Type == TokenType::T_TRUE || peek().value().Type == TokenType::T_FALSE)
	{
		...
		return std::make_shared<NumberLiteral>(valueToken.Type, valueToken.Value.value());
	}
	return nullptr;
}

std::shared_ptr<StringLiteral> Parser::parse_string_literal()
{
